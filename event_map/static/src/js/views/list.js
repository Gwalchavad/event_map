/*global define app server_time_tz*/
define([
    'jquery',
    'leaflet',
    'underscore',
    ...
    'jade!../../templates/EventClosedPartial',
    'jade!../../templates/liMonths',
    'jade!../../templates/liDays',
    'jade!../../templates/popup'], function (
        $,
        L,
        _,
        Backbone,
        ...
        var EventsListView = Backbone.View.extend({
                ...
                if (!self.backward_lock) {
                    ...
                    self.model.rfetch({
                        successCallback: function (events) {
                            ...
                            if (!self.nowFound) {
                                var index = self.renderNow();
                                if (index === -1)
                                    $("#EventsListView").scrollTop($("#EventsListView").height());
                            }
            ...
            onAdd: function (model, collect, options) {
                ...
                if (location_point) {
                    ...
                    direction_url = "https://www.google.com/maps?daddr=" + location_point.coordinates[1] + "," + location_point.coordinates[0];
                    if (map.currentPos) {
                        direction_url += "&saddr=" + map.currentPos.lat + "," + map.currentPos.lng;
                    }
                    model.set("direction_url", direction_url);
            ...
            renderEvent: function (position, events) {
                ...
                var html_months = temp_li_months({
                    months: [month_data]
                }),
            ...
            genarateColorsAndMonths: function (regenrate) {
                ...
                if (topVisbleEl.hasClass("event_item") && (regenrate || this.topVisbleEl[0] != topVisbleEl[0])) {
                    ...
                    if (this.colorEvents) {
                        //add color
                        for (i = startIndex; i <= numberOfEl + startIndex; ++i) {
                            marker = this.getMarker(i);
                            eventsViewed.push(marker[0]);
                            if (marker) {
                                var H;
                        ...
                        for (i = topVisibleIndex - 1 ; i > topVisibleIndex - this.numOfFades - 1; i--) {
                            if (i < nowIndex)
                                break;
                            marker = this.getMarker(i);
                            if (marker) {
                        ...
                        i = bottomIndex + 1 < nowIndex ? nowIndex :  bottomIndex + 1;
            ...
            setMonthSideBarPosition: function () {
                ...
                if (topElBottom > $("#EventsListView").position().top + topElwidth + tolarance) {
                    ...
                    if (new_height < 0)
                        new_height = 0;
                    //set to botto
                    current_top_month.addClass("relative").removeClass("monthFixed").css("top", new_height);
                }
            },
